...
import { formatDateTimeCN, formatDuration, METHOD_META, cn } from '@/lib/utils'
  ...
  return (
    ...
    >
      ...
      <div className="flex w-full flex-col gap-2 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        {/* Left — clicking the title area opens detail page */}
        <Link
          href={`/history/${session.id}`}
          className="flex min-w-0 items-start gap-3 sm:items-center hover:opacity-80 flex-1"
        >
        ...
        {/* Right: rating / verdict / duration / expand chevron */}
        <div className="flex flex-shrink-0 items-center gap-2 pl-0 sm:pl-4">
          ...
          <button
            ...
            aria-label="展开详情"
            className={cn(
              'text-gray-400 transition-transform duration-200 p-1 rounded hover:bg-gray-100',
      ...
      {/* Expanded detail — full rich view via SessionDetail */}
      {expanded && (
